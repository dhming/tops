...
__private.list = function (filter, cb) {
	...
	var processParams = function (value, key) {
		...
		if (_.includes(['fromUnixTime', 'toUnixTime'], field[1])) {
			// Tops epoch is 1464109200 as unix timestamp
...
Transactions.prototype.shared = {
	...
	addTransactions2: function (req, cb) {
		library.schema.validate(req.body, schema.addTransactions2, function (err) {
			if (err) {
				return setImmediate(cb, err[0].message);
			}

			if (!req.body.senderPublicKey) {
				return setImmediate(cb, 'Invalid senderPublicKey');
			}

			var query = {address: req.body.recipientId};
			var publicKey = req.body.senderPublicKey;
				...
				modules.accounts.getAccount(query, function (err, recipient) {
					...
					if (req.body.multisigAccountPublicKey && req.body.multisigAccountPublicKey !== publicKey) {
						modules.accounts.getAccount({publicKey: req.body.multisigAccountPublicKey}, function (err, account) {
							...
							modules.accounts.getAccount({publicKey: publicKey}, function (err, requester) {
								if (err) {
									return setImmediate(cb, err);
								}
						...
						modules.accounts.setAccountAndGet({publicKey: publicKey}, function (err, account) {
							...
							if (account.secondSignature && !req.body.signSignature) {
								return setImmediate(cb, 'Missing signSignature');
							}
