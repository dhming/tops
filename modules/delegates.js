...
__private.getBlockSlotData = function (slot, height, cb) {
	self.generateDelegateListAndTaxman(height, function (err, activeDelegates, taxmanPublicKey) {
		if (err) {
			return setImmediate(cb, err);
		}
		...
		for (; currentSlot < lastSlot; currentSlot += 1) {
			...
			if (delegate_id && __private.keypairs[delegate_id]) {
				return setImmediate(cb, null, {
					...
					taxmanPublicKey: taxmanPublicKey
				});
			}
		}
		...
		library.sequence.add(function (cb) {
			async.series({
				...
				if (err) {
					...
					return modules.blocks.process.generateBlock(currentBlockData.keypair, currentBlockData.time, currentBlockData.taxmanPublicKey, cb);
				}
			});
		}, function (err) {
			if (err) {
...
// Public methods
/**
 * Gets delegate list and taxman by vote and changes order.
 * @param {number} height
 * @param {function} cb - Callback function.
 * @returns {setImmediateCallback} err | truncated delegate list.
 * @todo explain seed.
 */
Delegates.prototype.generateDelegateListAndTaxman = function (height, cb) {
	__private.getKeysSortByVote(function (err, truncDelegateList) {
		if (err) {
			return setImmediate(cb, err);
		}

		var taxmanPublicKey = truncDelegateList[0];
		...
		return setImmediate(cb, null, truncDelegateList, taxmanPublicKey);
	});
};
...
Delegates.prototype.validateBlockSlot = function (block, cb) {
	self.generateDelegateListAndTaxman(block.height, function (err, activeDelegates, taxmanPublicKey) {
		if (err) {
			return setImmediate(cb, err);
		}
		...
		if (block.taxmanPublicKey !== taxmanPublicKey) {
			library.logger.error('Expected taxman: ' + taxmanPublicKey + ' Received taxman: ' + block.taxmanPublicKey);
...
Delegates.prototype.shared = {
	...
	getNextForgers: function (req, cb) {
		...
		modules.delegates.generateDelegateListAndTaxman(currentBlock.height, function (err, activeDelegates, taxmanPublicKey) {
			if (err) {
				return setImmediate(cb, err);
			}
			...
			return setImmediate(cb, null, {currentBlock: currentBlock.height, currentBlockSlot: currentBlockSlot, currentSlot: currentSlot, delegates: nextForgers, taxmanPublicKey: taxmanPublicKey});
		});
	},
