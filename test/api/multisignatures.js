...
function sendTOPS (account, i, done) {
	var randomTOPS = node.randomTOPS();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomTOPS / node.normalizer;
		}
		done();
	});
}

function sendTOPSFromMultisigAccount (password, amount, recipient, done) {
	node.put('/api/transactions/', {
		secret: password,
		amount: amount,
...
before(function (done) {
	sendTOPS(multisigAccount, null, done);
});
...
describe('PUT /api/multisignatures', function () {
	...
	it('using random passphase should fail', function (done) {
		...
		node.put('/api/multisignatures', validParams, function (err, res) {
			...
			node.expect(res.body).to.have.property('error').to.match(/Account does not have enough TYC: [0-9]+L balance: 0/);
			done();
		});
	});
	...
	describe('valid multisigs transactions with different min and keysgroup values', function () {
		...
		it('using valid params should be ok for min 1 and keysgroup array 1', function (done) {
			var params = {
				...
				lifetime: 11,
				min: 1,
				keysgroup: ['+' + node.tops.crypto.getKeys(node.randomPassword()).publicKey]
			};
		...
		it('using valid params should be ok when min 5 and keysgroup array 5', function (done) {
			var params = {
				...
				lifetime: 11,
				min: 5,
				keysgroup: Array.apply(null, Array(5)).map(function () { return '+' + node.tops.crypto.getKeys(node.randomPassword()).publicKey; })
			};
...
describe('PUT /api/transactions', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendTOPSFromMultisigAccount(multisigAccount.password, 100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
