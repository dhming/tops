...
describe('POST /peer/transactions', function () {
	...
	describe('enabling second signature', function () {
		...
		it('using undefined transaction.asset', function (done) {
			var transaction = node.tops.signature.createSignature(node.randomPassword(), node.randomPassword());
		...
		describe('when account has funds', function () {
			...
			it('should be ok', function (done) {
				var transaction = node.tops.signature.createSignature(account.password, account.secondPassword);
	...
	describe('using second signature', function () {
		...
		it('when account does not have one should fail', function (done) {
			var transaction = node.tops.transaction.createTransaction('1L', 1, node.gAccount.password, account.secondPassword);
		...
		it('using blank second passphrase should fail', function (done) {
			var transaction = node.tops.transaction.createTransaction('1L', 1, account.password, '');
		...
		it('using fake second signature should fail', function (done) {
			var transaction = node.tops.transaction.createTransaction('1L', 1, account.password, account.secondPassword);
