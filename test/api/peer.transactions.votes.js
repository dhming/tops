...
function postVotes (params, done) {
	...
	node.async.eachSeries(params.delegates, function (delegate, eachCb) {
		var transaction = node.tops.vote.createVote(params.passphrase, [params.action + delegate]);
...
function registerDelegate (account, done) {
	...
	var transaction = node.tops.delegate.createDelegate(account.password, account.username);
...
describe('POST /peer/transactions', function () {
	...
	it('using undefined transaction.asset', function (done) {
		var transaction = node.tops.vote.createVote(account.password, ['+' + delegate]);
	...
	it('using transaction.asset.votes containing invalid vote type', function (done) {
		var transaction = node.tops.vote.createVote(account.password, [0]);
	...
	it('using transaction.asset.votes containing invalid vote length', function (done) {
		var transaction = node.tops.vote.createVote(account.password, ['+' + delegate + 'z']);
	...
	it('using transaction.asset.votes containing manipulated vote', function (done) {
		var transaction = node.tops.vote.createVote(account.password, ['+8a6d629685b18e17e5f534065bad4984a8aa6b499c5783c3e65f61779e6da06czz']);
	...
	it('voting twice for a delegate should fail', function (done) {
		node.async.series([
			...
			function (seriesCb) {
				var transaction2 = node.tops.vote.createVote(account.password, ['+' + delegate]);
	...
	it(['voting for ', constants.maxVotesPerTransaction, 'delegates at once should be ok'].join(' '), function (done) {
		var transaction = node.tops.vote.createVote(account.password, delegates.slice(0, constants.maxVotesPerTransaction).map(function (delegate) {
			return '+' + delegate;
		}));
	...
	it(['voting for', constants.maxVotesPerTransaction + 1, 'delegates at once should fail'].join(' '), function (done) {
		var transaction = node.tops.vote.createVote(account.password, delegates.slice(0, constants.maxVotesPerTransaction + 1).map(function (delegate) {
			return '+' + delegate;
		}));
...
describe('POST /peer/transactions after registering a new delegate', function () {
	...
	it('voting for self should be ok', function (done) {
		var transaction = node.tops.vote.createVote(account.password, ['+' + account.publicKey]);
	...
	it('exceeding maximum of 101 votes should fail', function (done) {
		node.async.series([
			...
			function (seriesCb) {
				...
				var transaction = node.tops.vote.createVote(account.password, slicedDelegates.map(function (delegate) {
					return '+' + delegate;
				}));
