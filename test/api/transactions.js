...
function sendTOPS (account, amount, done) {
...
before(function (done) {
	setTimeout(function () {
		sendTOPS(account, node.randomTOPS(), done);
	}, 2000);
});

before(function (done) {
	setTimeout(function () {
		sendTOPS(account2, node.randomTOPS(), done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 20 TYC
		sendTOPS(account2, 20*100000000, done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 100 TYC
		sendTOPS(account2, 100*100000000, done);
	}, 2000);
});
...
describe('GET /api/transactions', function () {
	...
	it('using minAmount with and:maxAmount ordered by amount and limited should be ok', function (done) {
		...
		var offset = 0;
		var orderBy = 'amount:asc';
		var minAmount = 20*100000000; // 20 TYC
		var maxAmount = 100*100000000; // 100 TYC
	...
	it('using entire balance should fail', function (done) {
		openAccount({ secret: account.password }, function (err, res) {
			...
			putTransaction({
				...
				node.expect(res.body).to.have.property('success').to.be.not.ok;
				node.expect(res.body).to.have.property('error').to.match(/Account does not have enough TYC: [0-9]+L balance: [0-9.]+/);
				done();
			});
		});
	});
