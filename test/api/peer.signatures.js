...
describe('POST /peer/signatures', function () {
	...
	describe('creating a new multisignature account', function () {
		...
		before(function (done) {
			...
			node.async.eachSeries([owner, coSigner1, coSigner2], function (account, eachSeriesCb) {
				transactions.push(
					node.tops.transaction.createTransaction(account.address, 100000000000, node.gAccount.password)
				);
				eachSeriesCb();
			}, function (err) {
		...
		before(function (done) {
			...
			transaction = node.tops.multisignature.createMultisignature(owner.password, null, keysgroup, lifetime, min);
		...
		it('using processable signature for owner should fail', function (done) {
			var signature = node.tops.multisignature.signTransaction(transaction, owner.password);
		...
		it('using processable signature for coSigner1 should be ok', function (done) {
			var signature = node.tops.multisignature.signTransaction(transaction, coSigner1.password);
		...
		it('using processable signature for coSigner2 should be ok', function (done) {
			var signature = node.tops.multisignature.signTransaction(transaction, coSigner2.password);
	...
	describe('sending transaction from multisignature account', function () {
		...
		// Send transaction
		before(function (done) {
			transaction = node.tops.multisignature.createTransaction('1L', 1, owner.password);
