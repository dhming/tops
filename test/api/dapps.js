...
before(function (done) {
	// Send to TOPS to account 1 address
	setTimeout(function () {
		var randomTOPS = node.randomTOPS();
		var expectedFee = node.expectedFee(randomTOPS);
...
before(function (done) {
	// Send to TOPS to account 2 address
	setTimeout(function () {
		var randomTOPS = node.randomTOPS();
		var expectedFee = node.expectedFee(randomTOPS);
...
describe('PUT /api/dapps/withdrawal', function () {
	...
	beforeEach(function (done) {
		...
		var keys = node.tops.crypto.getKeys(randomAccount.password);
		var recipientId = node.tops.crypto.getAddress(keys.publicKey);
		var transaction = node.tops.transaction.createTransaction(randomAccount.address, 100000000, account.password);
	...
	it('using recipientId without an "L" should fail', function (done) {
		validParams.recipientId = validParams.recipientId.replace('T', '');
