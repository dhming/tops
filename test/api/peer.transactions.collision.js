...
describe('POST /peer/transactions', function () {
	...
	describe('when two passphrases collide into the same address', function () {
		...
		before(function (done) {
			...
			var transaction = node.tops.transaction.createTransaction(collision.address, 220000000, node.gAccount.password);
		...
		describe('when transaction is invalid', function () {
			...
			it('should fail for passphrase two', function (done) {
				var transaction = node.tops.transaction.createTransaction(node.gAccount.address, 100000000, collision.passphrases[1]);
			...
			it('should fail for passphrase one', function (done) {
				var transaction = node.tops.transaction.createTransaction(node.gAccount.address, 100000000, collision.passphrases[0]);
				transaction.signature = crypto.randomBytes(64).toString('hex');
				transaction.id = node.tops.crypto.getId(transaction);
