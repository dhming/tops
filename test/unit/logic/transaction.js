...
describe('transaction', function () {
	...
	describe('getBytes', function () {
		...
		it('should return same result of getBytes using /logic/transaction and tops-js package (without data field)', function () {
			var trsBytesFromLogic = transaction.getBytes(validTransaction);
			var trsBytesFromTopsJs = node.tops.crypto.getBytes(validTransaction);
			expect(trsBytesFromLogic.equals(trsBytesFromTopsJs)).to.be.ok;
		});
	...
	describe('checkBalance', function () {
		...
		it('should return error when sender has insufficiant balance', function () {
			...
			expect(res.error).to.include('Account does not have enough TYC:');
		});
	...
	describe('verify', function () {
		...
		it('should return error when account balance is less than transaction amount', function (done) {
			...
			createAndProcess(trsData, validSender, function (trs) {
				transaction.verify(trs, validSender, {}, function (err) {
					expect(err).to.include('Account does not have enough TYC:');
					done();
				});
			});
		});
