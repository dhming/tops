...
describe('multisignature', function () {
	...
	describe('objectNormalize', function () {
		...
		describe('min', function () {
			...
			it('should return error when value is not an integer', function () {
				var min = '2';
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, '+' + multiSigAccount2.publicKey], 1, 2);
			...
			it('should return error when value is smaller than minimum acceptable value', function () {
				...
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, '+' + multiSigAccount2.publicKey], 1, min);
		...
		describe('lifetime', function () {
			...
			it('should return error when value is smaller than minimum acceptable value', function () {
				...
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, '-' + multiSigAccount2.publicKey], lifetime, 2);
		...
		describe('keysgroup', function () {
			...
			it('should return error when it is not an array', function () {
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, [''], 1, 2);
			...
			it('should return error when array length is smaller than minimum acceptable value', function () {
				var keysgroup = [];
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, keysgroup, 1, 2);
			...
			it('should return error when array length is greater than maximum acceptable value', function () {
				var keysgroup = Array.apply(null, Array(constants.multisigConstraints.keysgroup.maxItems + 1)).map(function () {
					return '+' + node.tops.crypto.getKeys(node.randomPassword()).publicKey;
				});
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, keysgroup, 1, 2);
		...
		it('should return transaction when asset is valid', function () {
			var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, Array.apply(null, Array(10)).map(function () {
				return '+' + node.tops.crypto.getKeys(node.randomPassword()).publicKey;
			}), 1, 2);
	...
	describe('verify', function () {
		...
		describe('from transaction.verify tests', function () {
			...
			it('should return error when multisignature keysgroup has an entry which is null', function (done) {
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, null], 1, 2);
				trs.senderId = node.gAccount.address;
			...
			it('should return error when multisignature keysgroup has an entry which is undefined', function (done) {
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, undefined], 1, 2);
				trs.senderId = node.gAccount.address;
			...
			it('should return error when multisignature keysgroup has an entry which is an integer', function (done) {
				var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, 12], 1, 2);
				trs.senderId = node.gAccount.address;
	...
	describe('from multisignature.verify tests', function () {
		...
		it('should return error when min value is smaller than minimum acceptable value', function (done) {
			var min = constants.multisigConstraints.min.minimum - 1;
			var trs	= node.tops.multisignature.createMultisignature(node.gAccount.password, null, ['+' + multiSigAccount1.publicKey, '+' + multiSigAccount2.publicKey], 1, 1);
			trs.asset.multisignature.min = min;
