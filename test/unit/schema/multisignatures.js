...
describe('multisignatures', function () {
	...
	describe('addMultisignatures', function () {
		...
		beforeEach(function () {
			...
			testBody = {
				secret: secret,
				publicKey: node.tops.crypto.getKeys(secret).publicKey,
				min: 2,
				lifetime: 1,
				keysgroup: Array.apply(null, Array(4)).map(function () { return '+' + node.tops.crypto.getKeys(node.randomPassword()).publicKey;})
			};
		});
		...
		describe('keysgroup', function () {
			...
			it('should return error when keysgroup length is greater than maximum acceptable length', function () {
				testBody.keysgroup = Array.apply(null, Array(16)).map(function () { return node.tops.crypto.getKeys(node.randomPassword()).publicKey; });
				validator.validate(testBody, schema.addMultisignature);
				expect(validator.getLastErrors().map(function (e) {
					return e.message;
