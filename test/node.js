...
node.normalizer = 100000000; // Use this to convert TOPS amount to normal value
...
// Random TYC amount
node.TOPS = Math.floor(Math.random() * (100000 * 100000000)) + 1;
...
// Returns random TYC amount
node.randomTOPS = function () {
...
node.randomAccount = function () {
	...
	account.publicKey = node.tops.crypto.getKeys(account.password).publicKey;
	account.address = node.tops.crypto.getAddress(account.publicKey);
